// Command hellogo-init creates one mint and one token account for an already
// deployed HelloGo program, then mints an explicitly selected raw supply.
//
// HelloGo is the custom token program in examples/hellogo. Its accounts are not
...
import (
	...
	"github.com/ersany/go-solana/examples/hellogo"
	"github.com/ersany/go-solana/sdk"
	"github.com/ersany/go-solana/sdk/loader"
...
func main() {
	if err := runCLI(os.Args[1:], os.Stdout, os.Stderr, realDependencies()); err != nil {
		fmt.Fprintln(os.Stderr, "hellogo-init:", err)
		os.Exit(1)
	}
}

func runCLI(arguments []string, stdout, stderr io.Writer, deps dependencies) error {
	flags := flag.NewFlagSet("hellogo-init", flag.ContinueOnError)
	flags.SetOutput(stderr)
	programText := flags.String("program", "", "deployed HelloGo program address (required)")
	...
	if err != nil || programID == system.ProgramID || programID == loader.ProgramID {
		return fmt.Errorf("invalid deployed HelloGo program address %q", *programText)
	}
...
func initialize(
	...
	if config.Program == system.ProgramID || config.Program == loader.ProgramID {
		return nil, errors.New("invalid HelloGo program address")
	}
	...
	if err := emit(progressEvent{
		...
		AmountRaw: created.AmountRaw, AmountUI: created.AmountUI, Decimals: &created.Decimals,
	}); err != nil {
		return created, err
	}
	...
	mintRentLamports, err := client.MinimumBalanceForRentExemption(ctx, hellogo.MintStateSize)
	if err != nil {
		return created, fmt.Errorf("query mint rent: %w", err)
	}
	created.MintRentLamports = exactUint64(mintRentLamports)
	tokenRentLamports, err := client.MinimumBalanceForRentExemption(ctx, hellogo.TokenAccountStateSize)
	if err != nil {
	...
	if err := submit("create_and_initialize_mint", []svmtest.Signer{mint},
		system.CreateAccount(config.Payer.PublicKey, mint.PublicKey, mintRentLamports, hellogo.MintStateSize, config.Program),
		hellogo.InitializeMint(config.Program, mint.PublicKey, config.Payer.PublicKey, config.Decimals),
	); err != nil {
		return created, err
	}
	wantMint := hellogo.MintState{
		Initialized:   true,
		Decimals:      config.Decimals,
		MintAuthority: hellogo.OptionalPubkey{Set: true, Key: config.Payer.PublicKey},
	}
	...
	if err := submit("create_and_initialize_token_account", []svmtest.Signer{tokenAccount},
		system.CreateAccount(config.Payer.PublicKey, tokenAccount.PublicKey, tokenRentLamports, hellogo.TokenAccountStateSize, config.Program),
		hellogo.InitializeAccount(config.Program, tokenAccount.PublicKey, mint.PublicKey, config.Payer.PublicKey),
	); err != nil {
		return created, err
	}
	wantToken := hellogo.TokenAccountState{
		Initialized: true,
		Mint:        mint.PublicKey,
	...
	if err := submit("mint_to", nil,
		hellogo.MintTo(config.Program, mint.PublicKey, tokenAccount.PublicKey, config.Payer.PublicKey, config.AmountRaw),
	); err != nil {
		return created, err
	}
...
func verifyMint(ctx context.Context, client rpcClient, programID, address sdk.Pubkey, rent uint64, want hellogo.MintState) (hellogo.MintState, error) {
	data, err := verifiedStateData(ctx, client, programID, address, rent)
	if err != nil {
		return hellogo.MintState{}, err
	}
	got, err := hellogo.DecodeMintState(data)
	if err != nil {
		return hellogo.MintState{}, fmt.Errorf("decode mint: %w", err)
	}
	if got != want {
		return hellogo.MintState{}, fmt.Errorf("mint state %+v does not match expected %+v", got, want)
	}
	canonical := make([]byte, hellogo.MintStateSize)
	if err := hellogo.EncodeMintState(canonical, got); err != nil {
		return hellogo.MintState{}, fmt.Errorf("re-encode mint: %w", err)
	}
	if !bytes.Equal(data, canonical) {
		return hellogo.MintState{}, errors.New("mint data is not byte-for-byte canonical")
	}
	return got, nil
}

func verifyToken(ctx context.Context, client rpcClient, programID, address sdk.Pubkey, rent uint64, want hellogo.TokenAccountState) (hellogo.TokenAccountState, error) {
	data, err := verifiedStateData(ctx, client, programID, address, rent)
	if err != nil {
		return hellogo.TokenAccountState{}, err
	}
	got, err := hellogo.DecodeTokenAccountState(data)
	if err != nil {
		return hellogo.TokenAccountState{}, fmt.Errorf("decode token account: %w", err)
	}
	if got != want {
		return hellogo.TokenAccountState{}, fmt.Errorf("token-account state %+v does not match expected %+v", got, want)
	}
	canonical := make([]byte, hellogo.TokenAccountStateSize)
	if err := hellogo.EncodeTokenAccountState(canonical, got); err != nil {
		return hellogo.TokenAccountState{}, fmt.Errorf("re-encode token account: %w", err)
	}
	if !bytes.Equal(data, canonical) {
		return hellogo.TokenAccountState{}, errors.New("token-account data is not byte-for-byte canonical")
	}
	return got, nil
}
...
func mintStateOutput(state hellogo.MintState) mintStateJSON {
	authority := "disabled"
...
func tokenStateOutput(state hellogo.TokenAccountState, decimals uint8) tokenStateJSON {
	delegate := ""
