...
func writeAndDeploy(ctx context.Context, config Config, result *Result, buffer sdk.Pubkey, programRent uint64, elfBytes []byte) (*Result, error) {
	...
	for offset := 0; offset < len(elfBytes); offset += config.ChunkSize {
		...
		if err != nil {
			return result, fmt.Errorf("write buffer %s at offset %d; do not resend without inspecting finalized state: %w", buffer, offset, err)
		}
		if signature == "" {
			return result, fmt.Errorf("write buffer %s at offset %d returned no transaction signature", buffer, offset)
		}
	...
	if len(pendingWrites) != 0 {
		if err := config.Client.WaitForFinalized(ctx, pendingWrites[len(pendingWrites)-1].signature); err != nil {
			return result, fmt.Errorf("buffer write finalization checkpoint failed; do not resend without inspecting finalized state: %w", err)
		}
		bufferAccount, err := config.Client.GetAccountInfo(ctx, buffer)
		if err != nil {
			return result, err
		...
		if err != nil {
			return result, fmt.Errorf("verify finalized loader buffer %s: %w", buffer, err)
		}
		if !bytes.Equal(bufferData[loader.BufferMetadataSize:], elfBytes) {
			return result, errors.New("finalized loader buffer bytes do not match the strict ELF; refusing deploy")
		}
